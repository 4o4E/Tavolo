...
按顺序执行多个 handler。请求格式为 `multipart/form-data`：

- `image`：必填，初始输入图片或 GIF。
- `pipeline`：必填，JSON 字符串，类型为 `List<HandlerPipelineStep>`。
...
- `/pipelines/execute` 的多 handler 执行、空步骤和未知 handler 错误路径。
